...
public class MultipartOssHandler extends AbstractOssHandler {
    ...
    /**
     * Minimum size of part(100 * 1024kb)
     */
    private final long MIN_PART_SIZE = 102_400;
    ...
    public MultipartOssHandler(OssConfigs ossConfigs) {
        super(ossConfigs);
    }
    ...
    protected OssResourceDTO uploadActual(InputStream input, String resourceName, long resourceSize, String objectName,
        ...
        InitiateMultipartUploadRequest initiateMultipartUploadRequest = new InitiateMultipartUploadRequest(ossConfigs.getBucketName(), objectName);
//        initiateMultipartUploadRequest.setObjectMetadata();
        ...
        int partCount = (int) Math.ceil((double) resourceSize / MIN_PART_SIZE);

        CountDownLatch countDownLatch = new CountDownLatch(partCount);

        for (int i = 0; i < partCount; i++) {
            long startPos = i * MIN_PART_SIZE;
            try {
                input.skip(startPos);

                UploadPartRequest uploadPartRequest = new UploadPartRequest();
                uploadPartRequest.setBucketName(ossConfigs.getBucketName());
                uploadPartRequest.setKey(objectName);
                uploadPartRequest.setUploadId(uploadId);
                uploadPartRequest.setInputStream(input);
                uploadPartRequest.setPartSize((i + 1 == partCount) ? (resourceSize - startPos) : MIN_PART_SIZE);
        ...
        CompleteMultipartUploadRequest completeMultipartUploadRequest =
                new CompleteMultipartUploadRequest(ossConfigs.getBucketName(), objectName, uploadId, tags);
